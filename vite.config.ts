...
export default defineConfig(({ mode }) => {
  ...
  const GOOGLE_STT_KEY = env.GOOGLE_STT_API_KEY || env.VITE_GOOGLE_STT_API_KEY || ''
  ...
  const geminiProxy = () => ({
    ...
    configureServer(server: ViteDevServer) {
      ...
      server.middlewares.use('/api/eleven/tts', (req, res) => {
        // Quick sanity for missing env
        if (!ELEVEN_KEY) {
          res.statusCode = 500
          res.setHeader('Content-Type', 'application/json')
          res.end(JSON.stringify({ error: 'VITE_ELEVEN_API_KEY is missing in .env' }))
          return
        }

        const maskKey = (k: string) => (k ? `${k.slice(0, 4)}…${k.slice(-4)}` : 'empty')
        ...
        req.on('end', async () => {
          try {
            ...
            const url = `https://api.elevenlabs.io/v1/text-to-speech/${ELEVEN_VOICE}/stream?optimize_streaming_latency=4`
            const r = await fetch(url, {
              method: 'POST',
              headers: {
            ...
            // Log sanitized diagnostics in dev
            try { console.log(`[eleven-tts] voice=${ELEVEN_VOICE} key=${maskKey(ELEVEN_KEY)} status=${r.status}`) } catch {}

            if (!r.ok) {
              const errText = await r.text().catch(() => '')
              res.statusCode = r.status
              res.setHeader('Content-Type', 'application/json')
              res.end(errText || JSON.stringify({ error: 'ElevenLabs error', status: r.status }))
              return
            }
      ...
      // AssemblyAI: issue ephemeral token for realtime WS (dev only)
      server.middlewares.use('/api/assemblyai/token', async (_req, res) => {
        const AAI_KEY = env.ASSEMBLYAI_API_KEY || env.VITE_ASSEMBLYAI_API_KEY || ''
        if (!AAI_KEY) { res.statusCode = 500; res.end(JSON.stringify({ error: 'ASSEMBLYAI_API_KEY missing' })); return }
        try {
      ...
      // Google STT (sync recognize) - dev proxy
      server.middlewares.use('/api/google/stt', async (req, res) => {
        if (!GOOGLE_STT_KEY) { res.statusCode = 500; res.end(JSON.stringify({ error: 'GOOGLE_STT_API_KEY missing' })); return }
        try {
          let body = ''
          req.setEncoding('utf8')
          req.on('data', (chunk: string) => (body += chunk))
          req.on('end', async () => {
            try {
              const r = await fetch(`https://speech.googleapis.com/v1/speech:recognize?key=${GOOGLE_STT_KEY}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body,
              })
